                ...
                <div class="bg-black">
                    <div class="w-full bg-blue-500 p-6 rounded-lg">
                        ...
                        <div class="bg-white p-6 rounded-lg shadow-md">
                            <form action="{{ route('loan-collaterals.update', $loanCollateral->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="mb-4">
                                    ...
                                    <select name="loan_id" id="loan_id" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                        @foreach($loans as $loan)
                                            <option value="{{ $loan->id }}" {{ $loanCollateral->loan_id == $loan->id ? 'selected' : '' }}>
                                                {{ $loan->id }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea name="description" id="description" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" rows="3" required>{{ $loanCollateral->description }}</textarea>
                                </div>

                                <!-- Value -->
                                <div class="mb-4">
                                    <label for="value" class="block text-sm font-medium text-gray-700">Value</label>
                                    <input type="number" step="0.01" name="value" id="value" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="{{ $loanCollateral->value }}" required>
                                </div>
